...
Select Case przelacznik
	...
	Dim oPropertyValues 
	Set oPropertyValues = Vault.ObjectPropertyOperations.GetProperties( ObjVer,False )
	
	if (oPropertyValues.IndexOf (1459) = -1) then
		ID_Representative = 0
	else 
		ID_Representative = oPropertyValues.SearchForProperty( 1459 ).TypedValue.GetLookupID
	End if	
		 
	DIM ID_Customer, ID_Representative, ID_Country

	ID_Customer = oPropertyValues.SearchForProperty( 1457 ).TypedValue.GetLookupID

	ID_Country = oPropertyValues.SearchForProperty( 1507 ).TypedValue.GetLookupID


'		
	dodajDoLog("CUSTOMER - " & ID_Customer & " === REP - " & ID_Representative & " === Country - " & ID_Country )	
		
		
		
	call	NEW_SalesLead (ID_Customer, ID_Representative, ID_Country)
...
'	' Class = Manuals (general)
	...
	oOneSC.ConditionType = MFConditionTypeEqual
	oOneSC.Expression.SetPropertyValueExpression 1457,  MFilesAPI.MFParentChildBehavior.MFParentChildBehaviorNone, Nothing
	oOneSC.TypedValue.SetValue MFilesAPI.MFDataType.MFDatatypeLookup,  RepID
	oSCs_representative.Add -1, oOneSC
